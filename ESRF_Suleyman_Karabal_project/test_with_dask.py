...
### Path to ESRF data
path = '/media/vaggelis/Seagate Basic/'
data_path = path + 'NewBatch_SampleB_No4/'

### Output folder
output_folder = path + 'NewBatch_SampleB_No4_recon_with_CIL/'

num_of_tomogram = [25, 26]
...
for j in num_of_tomogram:
    ...
    data = da.from_array(f['entry_0000/measurement/data/'][:], chunks=(12, 2160, 2560)).astype(np.float32)            
    ...
    flat = da.from_array(f['entry_0000/measurement/data/'][:]).astype(np.float32)
    ...
    dark = da.from_array(f['entry_0000/measurement/data/'][:]).astype(np.float32)
    ...
    compute_center = dask.delayed(calc.find_center_360)(sino_360, 100, side=1)
    
    logging.info("Start Center of rotation correction")
    res = compute_center.compute()
    center0 = np.int32(np.round(res[0])) 
    ...
    for i in range(data.shape[1]):
        ...
        arr2 = dask.delayed(xRemoveStripesVertical)(arr1, decNum=10, wname='db25', sigma=0.1)
        results.append(arr2)

    logging.info("Start Remove vertical stripes")
    results_list = dask.compute(*results)
    logging.info("End Remove vertical stripes")    
